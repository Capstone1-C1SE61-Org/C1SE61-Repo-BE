...
public class InstructorInfo {
    ...
    public void validate(Object target, Errors errors) {
        InstructorInfo instructorInfo = (InstructorInfo) target;
        if (!(instructorInfo.dateOfBirth == null)) {
            LocalDate today = LocalDate.now();
            LocalDate minAgeDate = today.minusYears(12);
            LocalDate maxAgeDate = today.minusYears(90);
            if (instructorInfo.dateOfBirth.toLocalDate().isAfter(minAgeDate)) {
                errors.rejectValue("dateOfBirth", "", "Instructor must be at least 12 years old");
            }
            if (instructorInfo.dateOfBirth.toLocalDate().isBefore(maxAgeDate)) {
                errors.rejectValue("dateOfBirth", "", "Instructor age must not exceed 90 years");
            }
        }
    }
}
